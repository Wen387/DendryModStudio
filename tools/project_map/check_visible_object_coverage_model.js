...
assert(syntheticReport.summary.routeCoverage === 1, 'synthetic report should count manual boundaries as routed', syntheticReport.summary);
assert(syntheticReport.summary.goalW.passes70, 'synthetic report should pass Goal W despite router/manual boundaries', syntheticReport.summary.goalW);
assert(syntheticReport.rows.some((row) => row.area === 'news' && row.manualBoundary), 'news/router rows should stay visible as manual boundaries', syntheticReport.rows);
...
assert(syntheticReport.rows.some((row) => row.area === 'variables' && row.routeClass === 'variable_workspace'), 'variables should have a future Goal X route', syntheticReport.rows);
