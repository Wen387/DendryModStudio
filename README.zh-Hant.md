...
目前預覽版本為 `v0.97.6`。發布版本未經簽署，因此 Windows 可能會顯示 SmartScreen 警告。桌面發布包含專案索引器所使用的 Python 執行環境。
...
目前尚未提供官方 macOS 構建。你可以依照
[macOS setup guide](docs/macos-from-source.md)
從原始碼執行應用程式；文件會逐步說明，不需要事先熟悉 Terminal。
...
1. **Studio 版本** — 顯示於應用程式的「關於」或標題列（例如 `v0.97.6`）。
...
其他工程說明位於 [tools/project_map/README.md](tools/project_map/README.md)。發布準備說明位於 [docs/releases/v0.97.6-dev-preview.md](docs/releases/v0.97.6-dev-preview.md)，測試人員版本說明位於 [tools/project_map/RELEASE_NOTES_v0.97.6.md](tools/project_map/RELEASE_NOTES_v0.97.6.md)。
