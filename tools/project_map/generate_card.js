...
function usage() {
  return [
    'Usage: node tools/project_map/generate_card.js --draft <draft.json> --index <project-index.json> --out-dir <dir> [--summary]',
    '',
    'Generates an export-only CardDraft bundle. It never writes source files or applies patches.'
  ].join('\n');
}
...
function assertSafeOutDir(outDir, projectIndex) {
  ...
  if (isPathInside(resolved, root)) {
    fail('Refusing to write card export bundle inside the project repo; use a /tmp export directory.', 2);
  }
  [
    ...
    if (isPathInside(resolved, dir)) {
      fail('Refusing to write card export bundle under protected path: ' + path.relative(root, resolved), 2);
    }
  });
  return resolved;
}
...
function assertSafeRealOutDir(realOutDir, root) {
  if (isPathInside(realOutDir, root)) {
    fail('Refusing to write card export bundle inside the project repo real path; use a /tmp export directory.', 2);
  }
  [
    path.join(root, 'source'),
    path.join(root, 'out'),
    ...
    if (isPathInside(realOutDir, dir)) {
      fail('Refusing to write card export bundle under protected real path: ' + path.relative(root, realOutDir), 2);
    }
  });
}
...
function main() {
  ...
  const bundle = cardDraft.buildExportBundle(draft, index);
