(function initProjectMapElectionResultsSurface(global) {
  ...
  function renderToolbar(draft, sourceRows, selectedSource, collapsed) {
    return [
      ...
      '<p>' + escapeHtml(t('electionResults.intent', 'Choose an election event, edit the player-facing results screen, and keep chart data, conditions, effects, and consequences together.')) + '</p>',
      '<div class="election-results-metrics" data-election-results-metrics="true">',
      '<span>' + escapeHtml(String(sourceRows.length)) + ' ' + escapeHtml(t('electionResults.metric.events', 'candidate events')) + '</span>',
      ...
      '<button type="button" data-object-canvas-action="create_election_event">' + escapeHtml(t('electionResults.createElectionEvent', 'New election event')) + '</button>',
  ...
  function renderSelector(draft, sourceRows, selectedSource) {
    return [
      ...
      '<div class="template-eyebrow">' + escapeHtml(t('electionResults.selectedEvent', 'Selected election event')) + '</div>',
      '<label class="object-inline-field">',
      '<span>' + escapeHtml(t('electionResults.eventSelector', 'Election event')) + '</span>',
      ...
      sourceRows.length ? sourceRows.map((row) => '<option value="' + escapeAttr(row.id) + '"' + (row.id === draft.targetSceneId ? ' selected' : '') + '>' + escapeHtml(row.title || row.id) + '</option>').join('') : '<option value="">' + escapeHtml(t('electionResults.noElectionEvents', 'No indexed election event')) + '</option>',
      '</select>',
      '<small>' + escapeHtml(t('electionResults.eventSelectorHelp', 'Switch which existing election result event this draft inspects and modifies.')) + '</small>',
      '</label>',
      selectedSource ? renderSourceDetails(selectedSource) : '<p class="editing-empty">' + escapeHtml(t('electionResults.noSourceEventHelp', 'Create a new election event or enter a source target manually.')) + '</p>',
      '<button type="button" data-object-canvas-action="create_election_event">' + escapeHtml(t('electionResults.createElectionEvent', 'New election event')) + '</button>',
      '</section>'
    ].join('');
  }
