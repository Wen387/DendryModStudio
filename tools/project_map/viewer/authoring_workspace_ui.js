(function initProjectMapAuthoringWorkspace(global) {
  ...
  // as plain layers instead of six jargon categories. Buckets only reorder the
  // existing templates; every template stays individually reachable. The longer
  // layer explanation lives in the label tooltip so the row keeps menu-bar
  // height. A bucket with no templates yet (e.g. UI text before its editor
  // exists) is a roadmap entry and renders nothing.
  const SYSTEM_UI_BUCKETS = [
    {
      ...
      blurbFallback: 'The entry screen and game info are Dendry scenes underneath — edit them like ordinary content.',
    ...
    },
    {
      ...
      blurbFallback: 'Interface generated by the engine. Studio provides safe, extensible edit entries.',
    ...
    },
    {
      ...
      blurbFallback: 'Editing for variable-to-text display arrives in a later version.',
      templates: []
    }
  ];
  ...
  function renderBucket(bucket, templates) {
    ...
    const label = bucket
      ? '<span class="authoring-bucket-label" data-i18n="' + bucket.labelKey + '" data-i18n-title="' + bucket.blurbKey + '" title="' + bucket.blurbFallback + '">' + bucket.fallback + '</span>'
      : '';
    return '<div class="authoring-bucket" data-authoring-bucket="' + (bucket ? bucket.key : 'other') + '">' +
      label +
      templates.map((template) => renderTemplateButton(template, false)).join('') +
      '</div>';
  }
