...
function parentDebugScript(options) {
  ...
  return [
    ...
    'function varRow(item,pinned){var div=document.createElement("div");div.className="runtime-debug-row";div.setAttribute("data-debug-var-name",item.name);var info=document.createElement("div");info.innerHTML="<strong>"+esc(item.label||item.name)+"</strong> <span class=\\"runtime-debug-type\\">"+esc(item.valueType||"string")+"</span><small>"+esc(item.reason||item.summary||"")+"</small>";var wrap=document.createElement("div");wrap.className="runtime-debug-input-wrap";var input;if(item.valueType==="booleanNumber"){input=document.createElement("input");input.type="checkbox";input.className="runtime-debug-toggle";input.setAttribute("data-debug-variable-input",item.name);input.value="0";input.addEventListener("change",function(){this.value=this.checked?"1":"0";this.setAttribute("data-debug-dirty","1");});}else if(item.valueType==="number"){input=document.createElement("input");input.type="number";input.step="1";input.value="0";input.setAttribute("data-debug-variable-input",item.name);}else{input=document.createElement("input");input.type="text";input.maxLength=80;input.placeholder=L.textValuePlaceholder;if(item.knownValues&&item.knownValues.length){var dlId="dl-"+item.name;input.setAttribute("list",dlId);var dl=document.createElement("datalist");dl.id=dlId;item.knownValues.forEach(function(v){var opt=document.createElement("option");opt.value=v;dl.appendChild(opt);});wrap.appendChild(dl);}input.value="";input.setAttribute("data-debug-variable-input",item.name);}var pin=document.createElement("button");pin.type="button";pin.className="runtime-debug-pin"+(pinned?" is-pinned":"");pin.textContent=pinned?"\\u2605":"\\u2606";pin.title=pinned?L.unpin:L.pin;pin.setAttribute("data-debug-pin",item.name);wrap.appendChild(input);wrap.appendChild(pin);div.appendChild(info);div.appendChild(wrap);return div;}',
