(function initProjectMapExploreInspector(root) {
  ...
  function renderTextCorpusInspector(item, model, state, visibleEdit) {
    ...
    return [
      ...
      '<dt>' + escapeHtml(t('textCorpus.owner', 'Owner')) + '</dt><dd>' + escapeHtml([owner.kind, owner.sceneId || owner.itemId, owner.sectionId, owner.area].filter(Boolean).join(' / ')) + (ownerButton ? ' ' + ownerButton : '') + '</dd>',
