...
const SYSTEM_UI_CAPABILITY_MODEL = path.join(VIEWER, 'system_ui_capability_model.js');
const SYSTEM_UI_SEMANTIC_TASK_MODEL = path.join(VIEWER, 'system_ui_semantic_task_model.js');
...
const VIEWER_STYLE = path.join(VIEWER, 'styles.css');
...
const SYSTEM_UI_STYLE = path.join(VIEWER, 'styles', 'system-ui.css');
const LIGHTWEIGHT_OBJECT_PREVIEW_STYLE = path.join(VIEWER, 'styles', 'lightweight-object-preview.css');
...
const systemUiCapabilityModel = read(SYSTEM_UI_CAPABILITY_MODEL);
const systemUiSemanticTaskModel = read(SYSTEM_UI_SEMANTIC_TASK_MODEL);
...
const viewerStyle = read(VIEWER_STYLE);
...
const systemUiStyle = read(SYSTEM_UI_STYLE);
const lightweightObjectPreviewStyle = read(LIGHTWEIGHT_OBJECT_PREVIEW_STYLE);
...
assert(objectCanvasShellUi.includes('ProjectMapObjectCanvasShellUi'), 'Object Canvas shell helper should expose a browser API');
assert(objectCanvasShellUi.includes('data-authoring-surface'), 'Object Canvas shell helper should expose the active authoring surface');
assert(objectCanvasShellUi.includes('data-object-authoring-canvas'), 'Object Canvas shell helper should own the stable QA marker');
...
assert(objectDeleteProposalModel.includes('existing_scene_delete'), 'Object delete proposal model should preserve the delete proposal kind');
assert(objectDeleteProposalModel.includes('manual_review'), 'Object delete proposal model should keep deletes manual-review only');
...
assert(viewerStyle.includes('styles/preview-object-editor.css'), 'viewer styles should import the split Preview Object Editor stylesheet');
assert(previewObjectEditorStyle.includes('.preview-object-editor'), 'Preview Object Editor stylesheet should own preview-object editor styles');
assert(!editingStyle.includes('.preview-object-editor {'), 'Editing stylesheet should no longer own the main Preview Object Editor block');
...
assert(objectCanvasShellUi.includes('is-editor-overlay'), 'Object Canvas shell should support editor overlay mode');
...
assert(systemUiStyle.includes('.create-workspace.is-object-authoring .system-ui-layout'), 'System UI should shrink into the Create editing pane');
assert(lightweightObjectPreviewStyle.includes('.lightweight-object-preview'), 'Lightweight object preview stylesheet should include the lightweight object preview surface');
...
assert(systemUiRegionEditor.includes('data-system-ui-capability'), 'System UI region editor should render capability evidence cards');
assert(systemUiRegionEditor.includes('data-system-ui-runtime-state'), 'System UI region editor should expose runtime evidence markers');
...
assert(systemUiRegionEditor.includes('data-system-ui-content-scene-handoff'), 'System UI editor should hand source-backed scene text to the content editor before unrelated region fields');
...
assert(systemUiScreenPreview.includes('open_system_content_scene'), 'System UI preview should route player text and choices to the owning content scene');
...
assert(systemUiRegionEditor.includes('data-system-ui-theme-layout-candidate'), 'System UI region editor should expose limited theme/layout candidate markers');
assert(systemUiScreenPreview.includes('data-system-ui-install-safety'), 'System UI Screen preview should expose capability safety markers');
assert(canvasUi.includes('focusFieldId') && canvasUi.includes('replacementText'), 'Object Canvas should preserve Visible Edit System UI focus and replacement text');
...
assert(systemUiPreviewSurface.includes('data-system-player-flow-toolbar'), 'System UI surface should expose player screen modes');
...
assert(objectCanvasShellUi.includes('data-object-canvas-render-error'), 'Object Canvas shell should surface render failures instead of leaving stale content visible');
