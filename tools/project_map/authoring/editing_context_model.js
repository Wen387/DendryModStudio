...
(function initProjectMapEditingContextModel(global) {
  ...
  function editorFromField(field, values) {
    ...
    return {
      ...
      semanticRole: String(field.semanticRole || ''),
      ...
      conditions: ensureArray(field.conditions).map(String).filter(Boolean),
      routeKind: String(field.routeKind || ''),
      routeTarget: String(field.routeTarget || ''),
      routePredicate: String(field.routePredicate || ''),
      routeRaw: String(field.routeRaw || ''),
